...
export const levelForXP = (xp: number): SkillLevel => {
  for (let i = 0; i < xpLevels.length; i++) {
    const experience = xpLevels[i];

    if (experience > xp) {
      // We've passed the correct level (index `i - 1`)
      let previousLevelXP = 0;
      if (i > 0) {
        previousLevelXP = xpLevels[i - 1];
      }

      const xpBetweenLevels = experience - previousLevelXP;
      const xpPastLevel = xp - previousLevelXP;

      return {
        levelNumber: i,
        percentToNext: xpPastLevel / xpBetweenLevels,
      };
    }
  }
